...
class WhaleIntelligence:
    ...
    def rank_copytrade_candidates(
        ...
        """Surface whales worth copy-trading based on PnL, focus, consistency, and recency."""
        ...
        for row in cursor.fetchall():
            ...
            focus_ratio = row['crypto_focus_ratio'] or 0.0
            ...
            quality_score = edge_per_trade * recency * self._focus_multiplier(focus_ratio) * (1.0 + consistency)
    ...
    def backtest_copytrade_strategy(
        ...
        for row in wallets:
            ...
            consistency = self._compute_consistency(row['pnl_30d'] or 0.0, row['pnl_all'] or 0.0)
            focus_boost = self._focus_multiplier(row['crypto_focus_ratio'] or 0.0)
            latest_trade_ts = max((t['timestamp'] for t in trades), default=None)
            recency_boost = self._recency_multiplier(latest_trade_ts)
            ...
            risk_adjusted_expected = expected_pnl * focus_boost * (1.0 + consistency) * recency_boost
            total_expected += expected_pnl
            total_risk_adjusted += risk_adjusted_expected
            wallet_results.append(
                {
                    ...
                    'risk_adjusted_expected_pnl': risk_adjusted_expected,
                    'trade_count': len(trades),
                    'symbol_breakdown': symbol_breakdown,
                    'consistency': consistency,
                    'focus_multiplier': focus_boost,
                    'recency_multiplier': recency_boost,
                }
            )
    ...
    def export_ml_training_data(self, lookback_days: int = 30) -> List[Dict[str, Any]]:
        ...
        cursor.execute(
            '''
            SELECT ct.*, wp.pnl_30d, wp.pnl_all, wp.crypto_trade_count, wp.crypto_focus_ratio, wp.is_crypto_specialist
            FROM crypto_trades ct
            JOIN whale_profiles wp ON wp.wallet = ct.whale_wallet
            WHERE ct.timestamp >= ?
        ''',
    ...
    def print_copytrade_candidates(self, candidates: List[Dict[str, Any]]):
        ...
        for idx, c in enumerate(candidates, 1):
            ...
            print(
                ...
                f"Symbols: {c['symbol_count']} | Bias: {bias_text} | Edge/trade: ${c['edge_per_trade']:,.0f}"
            )
            print(
                f"    Quality: {c['quality_score']:.3f} | Recency x{c['recency_multiplier']:.2f} | Consistency {c['consistency']:.2f}"
            )
            if c.get('last_trade'):
...
def main():
    ...
    with WhaleIntelligence(db_path=db_path) as wi:
        ...
        if args.backtest_copytrade:
            ...
            print(
                f"Total expected PnL (model): ${backtest['total_expected_pnl']:,.0f} | "
                f"Risk-adjusted: ${backtest['total_risk_adjusted_pnl']:,.0f}\n"
            )
            for idx, wallet in enumerate(backtest['wallets'], 1):
                ...
                print(
                    ...
                    f"| risk-adj ${wallet['risk_adjusted_expected_pnl']:>10,.0f} | "
                ...
                print(
                    f"    Consistency {wallet['consistency']:.2f} | Focus x{wallet['focus_multiplier']:.2f} | Recency x{wallet['recency_multiplier']:.2f}"
                )
